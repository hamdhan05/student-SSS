    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-white">Homework Management</h2>
            ...
            {isCreateMode && (
                <div className="card p-6 border border-blue-500 border-opacity-30">
                    <h3 className="text-xl font-bold text-white mb-4">Assign Homework</h3>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                onChange={handleChange}
                                rows={4}
                                className="w-full px-4 py-2 rounded bg-white bg-opacity-10 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
            ...
            <div className="space-y-4">
                <div className="flex gap-4 items-center bg-gray-900 p-4 rounded-lg">
                    <span className="text-gray-400">View Homework For:</span>
                    <select
                        value={viewClass}
                ...
                {viewClass ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {homeworkList.map((hw: any) => (
                            <div key={hw.id} className="card p-6 hover:border-white transition-colors">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="px-3 py-1 rounded-full bg-purple-500 bg-opacity-20 text-purple-200 text-xs font-bold uppercase">
                                        {hw.subject}
                                    </div>
                                    <div className="text-sm text-gray-400">Due: {new Date(hw.dueDate).toLocaleDateString()}</div>
                                </div>
                                <h3 className="text-lg font-bold text-white mb-2">{hw.title}</h3>
                                <p className="text-gray-300 text-sm mb-4 line-clamp-3">{hw.description}</p>
